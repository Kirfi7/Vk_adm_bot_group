...
class Table:
    ...
    def get_default_info(self):
        return f"🔑 Основная информация 🔑\n" \
               f"Ваш никнейм: {self.array[1]}\n" \
               f"Должность: {self.array[2]}\n" \
               f"Доп. должность: {self.array[3]}\n" \
               f"Уровень админ-прав: {self.array[12]}\n" \
               f"\n📅 Важные даты и дни 📅\n" \
               f"Дата постановки: {self.array[4]}\n" \
               f"Последнее повышение: {self.array[11]}\n" \
               f"Дней с момента повышения: {self.array[18]}\n" \
               f"Всего дней на админ-посту: {self.array[19]}\n" \
               f"\n⛔️ Активные наказания ⛔️\n" \
               f"Количество выговоров: {self.array[14]}/3\n" \
               f"Количество предов: {self.array[15]}/2\n" \
               f"Количество устных: {self.array[16]}/2\n\n" \
               f"✅ Общее кол-во ответов: {self.array[17]}\n"

    def get_info_about_rank(self):
        admin_lvl = self.array[12]
        ...
        if int(admin_lvl) < 4:
            reports = int(self.array[17]); up_date = int(self.array[18])
            punish = [int(self.array[14]), int(self.array[15]), int(self.array[16])]
